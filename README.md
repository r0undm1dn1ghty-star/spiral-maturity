...
| `examples/case-battle-2026-09-16.md` | рыночный прогон (1-й цикл): 5 компаний разных отраслей, уровень × ступень + формат + один шаг |
| `examples/competition-ecommerce-2026-09-17.md` | рыночный прогон (2-й цикл): конкурентная разбивка 5 маркетплейсов e-commerce РФ |
...
| Компания | Уровень | Ступень | Уверенность | Инфра-поправка | Один шаг |
|---|---|---|---|---|---|
| Сбер | 7 Система | A4 Оркестрация | high | нет | Вывести ГигаКоворк за периметр группы |
| Т-Банк | 5 Достижение | A4 Оркестрация | high | нет | MCP-доступ к счёту как публичный API агентной экосистемы |
| Альфа-Банк | 5 Достижение | A3 Процесс | high | нет | Довести систему оценки агентов до kill-критериев |
| ВТБ | 4 Порядок | A2 Повторяемый эксперимент | medium | нет | Один процесс с агентом и kill-критерием до запуска платформы |
| РСХБ | 4 Порядок | A1 Пилот-инструмент | low | нет | Один процесс из «инструмента» в «агента в процессе» |

### Телеком

| Компания | Уровень | Ступень | Уверенность | Инфра-поправка | Один шаг |
|---|---|---|---|---|---|
| МТС | 5 Достижение | A3 Процесс | high | нет | Один сквозной клиентский сценарий под агентом с kill-критерием |
| Ростелеком | 4 Порядок | A3 Процесс | medium | нет | Масштабировать B2B-контур ИИ-автоматизации на второй сегмент |
| Мегафон | 5 Достижение | A2 Повторяемый эксперимент | medium | частично | Из двух ассистентов — один процесс с метрикой и владельцем |
| Билайн | 5 Достижение | A2 Повторяемый эксперимент | medium | да | Собственный процесс с агентом как референс для B2B-продаж |
| Yota | 3 Воля | A1 Пилот-инструмент | low | да | Один операционный отчёт под владельцем с метрикой |

### Ритейл-сети

| Компания | Уровень | Ступень | Уверенность | Инфра-поправка | Один шаг |
|---|---|---|---|---|---|
| X5 | 5 Достижение | A3 Процесс | high | нет | Из пилотов (Умный пекарь, CoPilot) в стандарт процесса сети |
| Магнит | 5 Достижение | A2 Повторяемый эксперимент | medium | частично | Прогноз спроса — процесс с владельцем и регламентом контура |
| Лента | 4 Порядок | A2 Повторяемый эксперимент | medium | да | Один процесс с агентом, метрикой и владельцем |
| Ашан РФ | 3 Воля | A1 Пилот-инструмент | low | да | Один болезненный процесс под владельцем с kill-критерием |
| Перекрёсток | 5 Достижение | A2 Повторяемый эксперимент | medium | да | «Умный пекарь»: метрика списаний → масштабировать или закрыть |

**Проверено в 3-м цикле:**

1. **Факты до оценки** — снова: стереотипы «банк = бюрократия» и «ритейл = конвейер» сломались о факты (Сбер 7, X5 5).
2. **Нижний разрыв (застой)** — новый паттерн: РСХБ 4/A1, потолок A2–A3 не использован; лечение — не платформа, а один процесс с метрикой и владельцем.
3. **Ступень на потолке** — Ростелеком 4/A3: разрыва нет, но рост в A4 блокирован логикой согласований уровня 4.
4. **Уверенность как диагностический факт** — low у РСХБ, Ашана и Yota (молчание в публичике) — само по себе сигнал индикатора 5.

> Полные разборы — входные факты с URL и датами, 16 индикаторов, мост, adversarial self-check: [`examples/competition-banks-2026-09-20.md`](examples/competition-banks-2026-09-20.md), [`examples/competition-telecom-2026-09-20.md`](examples/competition-telecom-2026-09-20.md), [`examples/competition-retail-2026-09-20.md`](examples/competition-retail-2026-09-20.md)
